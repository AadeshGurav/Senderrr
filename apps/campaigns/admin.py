...
class WhatsAppGroupAdmin(admin.ModelAdmin):
    ...
    list_display = ("name", "group_jid", "is_active", "created_at")
...
class MessageTaskInline(admin.TabularInline):
    ...
    model = MessageTask
    extra = 0
    readonly_fields = ("group", "status", "error_message", "queued_at", "sent_at")
    can_delete = False
...
class MessageTaskAdmin(admin.ModelAdmin):
    ...
    list_display = ("pk", "broadcast", "group", "status", "queued_at", "sent_at")
    list_filter = ("status",)
